...
*** Configuration
=rofi-borg= is designed to be easily configured from the top level script and the settings are passed into subsequent scripts. Configuring =rofi-borg.sh=, =backup_run.sh= backup directories, and setting your repository and password in =$HOME/.borg-repo= and =$HOME/.borg-passphrase= is the minimal amount of configuration required. However, I recommend you checkout every script to make sure it's setup how you like.
