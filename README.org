...
*** Configuration:
=rofi-borg= is designed to be easily configured from the top level script and the settings are passed into subsequent scripts. That is the minimal amount of configuration required. All borg command options are set per script, and rofi-borg ships with sane defaults.
