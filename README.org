...
- [X] Add a check for logging must be >= 1, never 0. And add a comment to never put to 0.
...
- [ ] Fix bug where if no selection do nothing or escape/click exit... instead of run first available command.
  - Is related to always running backup_run then reporting it. Make some form of check for selection then do.
