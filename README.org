...
- Customizable:
  ...
  - There is a main theme config, edit that to change the primary theming. Certain scripts have their own configs which inherit from the main config. This allows theming to be configured from the main file, or to change theme/size per script independently. See backup_run.sh and backup_run.rasi for an example.
- Extendable:
