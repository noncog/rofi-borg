...
**** rofi-borg.sh
Contins:
- global options set for all sub-scripts
  - Minimally must set:
...
- Contains all borg command options. rofi-borg ships with sane defaults, but can customize these for a particular borg command in it's related script.
