...
- [ ] Add to tips for contributing/extending that selection needs escaped. Best bet is to review the given scripts and completely model yours after the one that best fits.
- [ ] Add a note that users must set their file directories: downloads, etc.
...
- [ ] See man rofi-theme for all theming related information.
- [ ] Add a note that if you don't want the theme I provide at all, then just rewrite the config files to what you want, so long as you keep the same names.
